...
const DiscordComponent = () => {
    ...
    const copyToClipboard = () => {
      copy(copyText);
      alert("Command copied! Paste into Discord to link your account.");
    }
    ...
    async function run(uuid) {
        if (window.ethereum) {
            ...
            if (!(uuid == null)) {
              web3.eth.personal.sign("I agree that the account signing this message will be linked to the Discord account with the ID of " + id, accounts[0], "").then(
                  (msg) => {
    ...
    return (
        <Wrapper>
            <p className="username">Link Your Discord Account</p>
