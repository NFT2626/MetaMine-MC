...
const StakeComponent = () => {
    ...
    const [tokenReward, setTokenReward] = useState(0.434026211328);
    ...
    const handleTokenReward = (e) => {
        setTokenReward(totalTokenReward(e.target.value));
    }

    const handleBothTokenRewardAndTime = (e) => {
        handleTime(e);
        handleTokenReward(e);
    }
  ...
  async function lock() {
    ...
    if (window.ethereum) {
      ...
      var contract = new web3.eth.Contract(abi, '0x6231DcfF2569214615bdacA9EA7d5C3946608920');
  ...
  const getTokenLockInfo = async () => {
    ...
    .then((isLocked) => {
      ...
      .then((tokenLockInfo) => {
        ...
        .then((block) => {
          const endDate = new Date(Number(tokenLockInfo[1]) * 1000);
          const button = document.getElementsByClassName("button")[0];
          const warning = document.getElementsByClassName("warning")[0];
          const warning2 = document.getElementsByClassName("warning2")[0];
          const dateP = document.getElementsByClassName("date")[0];
          if (((tokenLockInfo[1] < block.timestamp) && (!isLocked)) || tokenLockInfo[1] == null) {
            ...
            warning.style.display = "flex";
            warning2.innerHTML = "Please claim your token reward for this plot <b>BEFORE</b> unlocking the plot. You may lose your claim if you unlock before claiming.";
          } else {
  ...
  async function run() {
    ...
    if (window.ethereum) {
      const web3 = new Web3(window.ethereum);
      web3.eth.subscribe('newBlockHeaders', getTokenLockInfo);
    }
  }
  

  function totalTokenReward(t) {
      return 64602*(10**-18)*((2592000*t)**2);
  }
